...
Overhead would change with each CUDA variant since each variation would have a different problem to tackle.<br>
The Overhead is explained below:<br>
- CUDA unified
  - kernel + Host to Device + Device to Host
  - Page is not in GPU's frames, resulting in a page fault, hence the Host to Device overhead.
- CUDA Prefetch
  - kernel + Device to Host
  - Needed pages are moved to the GPU's frames before kernel run, removing the page fault.
...
All variations have the Device to Host overhead since they have to transfer the data from GPU to CPU once the kernel is done executing.
